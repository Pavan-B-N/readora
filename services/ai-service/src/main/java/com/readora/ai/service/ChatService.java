...
// The MCP tools that read a specific user's own data (getCart, getOrderHistory, etc.) declare userId as a parameter, but the model must never be trusted to supply it — see ChatClientConfig's userScopedToolCallbacks() bean; each chat request wraps those callbacks with the caller's real, JWT-authenticated user id (never from request.message() or anything the model produced) before making them available for this one call.
@Service
public class ChatService {

    private static final Logger log = LoggerFactory.getLogger(ChatService.class);

    // A stream frame prefixed with this is metadata, not a token to display — the frontend strips it out of the visible reply and parses the rest as the bookIds JSON array; content-wise this can never collide with real model output since it's not natural-language text.
    ...
    // Wires the chat client, persistence, tool callbacks, and reference-verification collaborators.
    public ChatService(
    ...
    // Streams a chat reply for one turn, persisting both the user message and the eventual assistant reply plus a trailing bookIds frame.
    public ChatStream chat(UUID userId, ChatRequest request) {
        Conversation conversation = resolveConversation(userId, request);

        // Fetched before saving the new user message below, so this is exactly the prior turns — the new message is added separately via .user() a few lines down, not duplicated here; without this the model only ever sees the single latest message, so a follow-up like "which of these is best" would have no idea what "these" refers to, replacing an earlier, unreliable reliance on the model voluntarily calling a "load my own history" tool.
        ...
        // The book-retrieval guardrail: every candidate ReadoraInternalTools hands back is already filtered to what's actually purchasable at this store — see its filterAvailable() and StoreScopedToolCallback's own docs for why the model's own storeId value is never trusted.
        ...
        // Peels the leading marker off before any of it reaches doOnNext/the client, so the user only ever sees the clean reply — never a flash of the bookkeeping comment.
        ...
        // One extra frame appended after the real tokens: persists the assistant message (its full text is only known once streaming completes) and hands the same book ids — the ones the model itself cited, not a fresh guess — to the client in the same stream, so a fresh reply and a reloaded one both carry them the same way.
        Flux<String> tokens = textTokens.concatWith(Mono.fromCallable(() -> {
            ...
            // Belt-and-suspenders on top of the tool-level guardrail: closes the gap where the model recalls a book by name from earlier in the conversation without a fresh, already-filtered tool call, e.g. after the caller switched stores mid-conversation — every id shown to the user passes this check, no exceptions.
    ...
    // Resumes an existing conversation the caller owns, or starts a new one when none was given.
