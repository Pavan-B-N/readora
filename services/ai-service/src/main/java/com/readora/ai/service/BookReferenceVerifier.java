...
// Re-verifies, after the model has finished replying, that every book id it cited both got discussed in the actual reply text and is still available at the requesting store — split out of ChatService so these two independent guardrails (discussed-in-reply vs. still-available) are testable without also standing up conversation persistence or the model client.
@Component
public class BookReferenceVerifier {
    ...
    // Wires the vector store and catalog client used to verify cited book ids.
    public BookReferenceVerifier(VectorStore vectorStore, CatalogClient catalogClient) {
        this.vectorStore = vectorStore;
        this.catalogClient = catalogClient;
    }

    // A backstop against the model padding the marker with every candidate it retrieved rather than only the ones it actually wrote about — observed happening in practice despite the prompt's explicit cardinality rule; keeps only ids whose real title actually appears in the reply text, so "the model looked at 5 books but only wrote about 3" can't leave 2 unrelated cards in the response.
    ...
    // The final guardrail before anything reaches the client: re-checks store availability on whatever ids survived filterToActuallyDiscussedIds, independent of whether they came from a tool call this turn or the model recalling earlier conversation history; unlike CatalogClient.checkAvailability's own fail-closed default, an id dropped here just means one fewer card since the reply text itself was already finalized and isn't retried.
