...
// Embeds a virtual edition's PDF content for the in-reader RAG assistant into its own pgvector table, deliberately separate from the catalog-search vector_store (whole pages of book content vs. title/description/topics for product search) and, since a second Spring-managed VectorStore bean of the same type would create an unqualified-injection ambiguity for every consumer of the primary one, built and owned privately here instead of as a @Bean; a book is only ever embedded once, on whichever purchaser opens the reader first, since the file is identical for every owner and re-embedding per reader would just waste embedding-API calls.
@Service
public class BookContentIndexService {
    ...
    // Wires the dependencies used to build and query the book-content vector store.
    ...
    // Builds and initializes the private book-content pgvector store on startup.
    @PostConstruct
    void init() {
    ...
    // Looks up a book's current reader-index status, or null if never indexed.
    ...
    // Fetches the book's file, extracts its text, chunks it, and embeds every chunk (a no-op if already ready); ownership is checked here, not just at chat time, so indexing itself — real embedding-API cost — can only ever be triggered by an actual purchaser.
    @Transactional
    public BookReaderIndexStatus initialize(UUID userId, UUID bookId) {
        ...
        try {
            ...
            for (int i = 0; i < chunks.size(); i++) {
                ...
                // The vector table's id column is a real uuid type (same schema shape as the catalog vector_store), so a composite string like "<bookId>-chunk-<i>" isn't a valid UUID and fails at insert; retrieval is always by the bookId metadata filter below, never by looking a document up by its id, so the id itself just needs to be unique.
    ...
    // Package-private (not private) so BookContentIndexServiceTest can exercise the chunking policy directly, without needing a real PDF or the PgVectorStore built in init().
    List<String> extractChunks(byte[] pdfBytes) throws Exception {
        ...
        for (String paragraph : paragraphs) {
            ...
            // A paragraph by itself can exceed CHUNK_SIZE (e.g. a long unbroken bullet list with no blank-line breaks), and the greedy packing below only ever prevents *combining* paragraphs past the limit, so an oversized one on its own must be hard-split first or it reaches the embedding model as a single document over its input-token limit and the whole run fails.
