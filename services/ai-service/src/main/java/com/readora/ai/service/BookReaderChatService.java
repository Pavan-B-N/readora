...
// The reader's AI assistant — deliberately a plain retrieval-augmented Q&A over one book's own content, not the general shopping assistant (ChatService): no tool calling, no book recommendations, no access to other books, a focused reading companion that only ever answers from what's actually in the book the reader has open.
@Service
public class BookReaderChatService {
    ...
    // Builds its own ChatClient from a fresh builder chain, not the shared `chatClient` bean — this assistant must never inherit the shopping assistant's tools or system prompt.
    ...
    // One reader-chat message as exposed to callers.
    public record ReaderMessage(String role, String content) {
    }

    // Returns the reader chat history for a book, oldest first.
    ...
    // Answers a reader's question about their open book, persisting both the question and the reply.
    @Transactional
    ...
    // Throws BookAccessDeniedException unless the user actually owns the book.
