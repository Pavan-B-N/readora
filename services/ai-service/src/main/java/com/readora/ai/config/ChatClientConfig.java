...
// Wires internal tools (in-process) and the MCP client's tools (mcp-server, over streamable HTTP — the ToolCallbackProvider bean here is auto-configured by spring-ai-starter-mcp-client from spring.ai.mcp.client.* properties) into one ChatClient. Unverified against a live build — see build notes.
@Configuration
public class ChatClientConfig {

    // mcp-server tool names that read a specific user's own data and declare userId as a parameter; deliberately excluded from the shared ChatClient's default tools below — see userScopedToolCallbacks() for why.
    private static final Set<String> USER_SCOPED_TOOL_NAMES =
            Set.of("getCart", "getOrderHistory", "getUserProfile", "getWalletBalance");

    // Two jobs: (1) get the model writing real Markdown, since the frontend renders it as such rather than a flat text blob; (2) get an accurate, self-reported list of which book ids a reply actually recommends via a leading hidden marker that ChatService.stripLeadingReferenceMarker peels off, replacing an earlier approach of re-running a fresh vector search after the fact — which routinely surfaced books the reply never mentioned, since "semantically near the question" and "what the model chose to recommend" are different things, and self-reporting is the only way to know which candidates it settled on.
    ...
    // Builds the shared ChatClient with the system prompt and the safe (non-user-scoped) MCP tool callbacks.
    @Bean
    ...
    // The four MCP tools that read one specific user's own data; not registered as defaults on the shared ChatClient bean above because each takes userId as a model-visible parameter, which would let the model choose whose cart, orders, profile, or wallet to read — ChatService instead wraps each per-request in a UserScopedToolCallback that overwrites userId with the caller's real, JWT-authenticated id before the call goes out.
    @Bean
    ...
    // ReadoraInternalTools' @Tool-annotated methods, converted to plain ToolCallbacks so ChatService can wrap each in a StoreScopedToolCallback per request — see the chatClient() bean above for why these aren't registered as shared defaults; declared as an array rather than List<ToolCallback> because Spring's ToolCallingAutoConfiguration autowires a single List<ToolCallback> bean for its internal tool resolver, and having two beans of that exact type (this one and userScopedToolCallbacks above) made that injection ambiguous and broke application startup — an array is a different declared type, so it no longer competes for that slot.
    @Bean
