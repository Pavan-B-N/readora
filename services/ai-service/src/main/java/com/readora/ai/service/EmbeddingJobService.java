...
// Backfills run asynchronously off a Kafka event rather than inline in the HTTP request, since a full re-embed calls an external embedding API once per page and can take minutes, far too long to hold a connection open — the request only enqueues a job and returns its id, and the consumer does the work and writes progress back so the UI can poll.
@Service
public class EmbeddingJobService {
    ...
    // Wires the job/log repositories, embedding service, Kafka template, and JSON mapper.
     ...
     * @throws BackfillAlreadyRunningException if one is already queued or running, since concurrent backfills would duplicate expensive embedding-API calls for no benefit
     */
    @Transactional
    ...
    /**
     * Runs a queued backfill to completion. Called by the Kafka consumer, never directly by a request thread.
     *
    ...
    // Progress updates commit in their own transaction so the admin UI can poll and see them while the (long-running) backfill is still in flight; logs every book in the just-processed page individually, not just the last one, so the detail page can show a real book-by-book feed rather than only an aggregate counter.
    ...
    // Applies a status-mutating change to a job, in its own transaction.
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    ...
    // Finds the currently queued or running job, if any.
    @Transactional(readOnly = true)
    ...
    // Maps an EmbeddingJob entity to its response DTO.
    ...
    // Walks to the root cause of an exception and truncates its message to 500 chars.
