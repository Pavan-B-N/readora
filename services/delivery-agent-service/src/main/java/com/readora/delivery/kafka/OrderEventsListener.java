...
// Consumes order status change events to queue delivery and return pickup assignments.
@Component
public class OrderEventsListener {

    private static final Logger log = LoggerFactory.getLogger(OrderEventsListener.class);

    // Payout scales with order value and item count instead of a flat rate — base fee plus a capped value-based bonus (proxy for distance/risk, since this build has no geo-distance data) plus a small per-extra-item bonus; a pickup pays less than a full delivery since it's the simpler leg.
    ...
    // Wires the repositories, commerce client, and object mapper used to build assignments.
    ...
    // Queues a delivery when a physical order reaches CONFIRMED, or a pickup when its return is approved; ignores every other status/order type, and is idempotent via findByOrderId.
    ...
    // Creates a return pickup assignment for an order whose return was just approved.
    ...
    // Holds the commerce-service delivery detail fields needed to snapshot an assignment at creation.
    ...
    // Computes the agent payout from a base rate plus capped value and item-count bonuses.
    ...
    // Best-effort, resolved once here rather than on every queue read — a missing value is just a cosmetic gap until the agent opens the live detail page, not worth failing assignment creation over a brief outage.
    private DeliveryDetailSnapshot resolveSnapshot(UUID orderId) {
        try {
            ...
            // Field names match OrderDeliveryDetailResponse.Item exactly, so this round-trips cleanly through ItemSnapshot on the read side with no manual mapping.
