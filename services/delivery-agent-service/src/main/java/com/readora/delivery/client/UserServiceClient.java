...
public class UserServiceClient {
    ...
    // Resolves which store an admin is scoped to — fail-closed: any error here propagates rather than being treated as "no store," so an admin can never fall through to seeing every store's agents just because user-service hiccuped.
    ...
    // Distinguishes a real rejection from user-service (its own status code and message, passed straight through) from an actual connectivity failure, which alone deserves the generic "unavailable" message.
