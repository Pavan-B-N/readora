...
// Direct service-to-service calls to commerce-service, bypassing api-gateway — same pattern as catalog-service's internal clients; commerce-service's Order.status stays the source of truth, this service only ever reads it or asks it to advance.
@Component
public class CommerceClient {
    ...
    // Builds the RestClient with the configured base URL and gateway secret header.
    ...
    // Fetches the delivery detail for an order from commerce-service.
    ...
    // Pushes an updated delivery status for an order to commerce-service.
    ...
    // Pushes an updated return status for an order to commerce-service.
    ...
    // Distinguishes a real rejection from commerce-service (its own status code and message, passed straight through — e.g. a 409 because the order already moved past this status) from an actual connectivity failure (commerce-service unreachable/timed out), which alone deserves the generic "unavailable" message — collapsing both into 503 previously made every upstream validation error look like an outage.
    ...
    // Pulls the "message" field out of a JSON error response body, if present.
