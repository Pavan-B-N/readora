...
public class ReturnPickupService {
    ...
    // Returns a pickup plus its live order detail from commerce-service, scoped to the caller's store.
    @Transactional(readOnly = true)
    ...
    // Claims an UNASSIGNED pickup for the caller and notifies commerce-service, or rejects if another agent already won the race.
    @Transactional
    ...
    // Advances the caller's ASSIGNED pickup to EN_ROUTE and pushes the status to commerce-service.
    @Transactional
    ...
    // Advances the caller's EN_ROUTE pickup to COLLECTED and pushes the status to commerce-service.
    @Transactional
    ...
    // Maps a pickup assignment entity to its API response shape, parsing the stored items JSON.
