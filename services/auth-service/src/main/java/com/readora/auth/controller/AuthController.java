...
// REST endpoints for registration, login, refresh-token rotation, and logout.
@Tag(name = "Auth")
@RestController
...
public class AuthController {
    ...
    // Injects the auth service used to fulfill every endpoint below.
    ...
    // Creates a new account and returns the created response.
    ...
    // Authenticates the caller and issues a new access/refresh token pair.
    @PostMapping("/login")
    ...
    // Rotates a refresh token, revoking all active tokens on reuse detection.
    @PostMapping("/refresh")
