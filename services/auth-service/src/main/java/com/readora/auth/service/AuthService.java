...
public class AuthService {
    ...
    // Wires the repositories, security helpers, and tunable limits this service needs.
    public AuthService(
    ...
    // Creates a new account with a hashed password and the default CUSTOMER role.
    @Transactional
    ...
    // Unknown email and wrong password both fail with the same InvalidCredentialsException so the endpoint can't be used to enumerate registered accounts.
    @Transactional
    ...
    // The submitted token is revoked before the replacement pair is issued, so a stolen token can only ever be used once; if a revoked token is presented again (reuse), every other active token for that user is revoked too, killing the whole session.
    @Transactional
    ...
    // Unlike password hashing, this doesn't need to be slow or salted — the raw value is already 128 bits of random entropy (a UUID), so a fast deterministic hash is both sufficient and required for indexed exact-match lookup by hash.
