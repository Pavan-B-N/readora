...
// Kafka consumer that turns domain events from other services into persisted, pushed notifications.
@Component
public class NotificationEventsListener {
    ...
    // Wires in the notification service and JSON mapper.
    public NotificationEventsListener(NotificationService notificationService, ObjectMapper objectMapper) {
        this.notificationService = notificationService;
        this.objectMapper = objectMapper;
    }

    // Single source for every order-lifecycle notification (placed, paid, confirmed, shipped, delivered, cancelled, payment failed) — commerce-service publishes this on every status transition, so this listener covers the whole order journey generically rather than needing a bespoke Kafka listener per status.
    ...
    // Notifies a user that their refund has been processed.
    ...
    // Generic, arbitrarily-targeted notifications — currently used for a store admin's "return awaiting review" alert and both sides' "new return chat message" alert. Unlike onOrderStatusChanged above, the sender already rendered title/message, so this just passes them straight through.
    ...
    // Renders the title/message pair for an order status transition.
