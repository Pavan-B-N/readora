...
/** Dummy payment provider: every authorization/capture succeeds immediately, no real gateway call. This is intentional per project scope, not a placeholder left unfinished — replacing it with a real provider integration later only touches this class. */
@Service
public class PaymentService {
    ...
    // Wires the repositories and mapper this service needs.
    ...
    /** WALLET checkout already had its balance verified synchronously by commerce-service before the order was created, so it's safe to capture immediately here. UPI has no real gateway — it's simulated: authorize now, and {@link UpiSettlementJob} captures it a few seconds later, entirely server-side over Kafka, so the frontend sees a genuine "pending, then confirmed" flow rather than a fake client-side timer. */
    @Transactional
    ...
    // Publishes a PaymentCapturedEvent via the outbox for this payment.
    ...
    // Refunds the payment for a cancelled order.
    @Transactional
    ...
    // Shared refund path for both cancellation and return events, guarded against redelivery.
    private void refund(UUID orderId, UUID userId, String reason, BigDecimal refundAmount) {
        ...
        // Kafka delivers order.cancelled/order.returned at-least-once — without this guard, a redelivery would mint a second Refund row (fresh UUID) and double-credit the wallet, since downstream idempotency in user-service dedupes by refundId, which would differ.
    ...
    /** Internal-only lookup — no ownership check. Safe only because the sole caller, InternalPaymentController, is reachable exclusively service-to-service (GatewaySecretFilter, not per-caller auth) and the calling service (commerce-service) has already verified the requesting user owns the order before asking for its payment details. The public endpoint (PaymentController) must never call this overload directly — see getByOrderId(orderId, callerId). */
    ...
    /** Ownership-checked variant for the public /api/v1/payments/{orderId} endpoint. Without the ownership filter here, any authenticated user could view another user's payment amount, wallet-funded amount, method, and status just by passing an arbitrary order id. Failing with the same PaymentNotFoundException (404) for both "no such payment" and "not yours" means a caller can't distinguish the two by probing order ids. */
    @Transactional(readOnly = true)
    ...
    // Maps a Payment entity to its API response shape (refund info left null here).
    ...
    /** Batch lookup for commerce-service's admin returns view — one round trip for a whole page of orders rather than N+1 internal calls. Orders with no refund row yet (event not processed by this service, or none applicable) are simply absent from the result. */
    @Transactional(readOnly = true)
    ...
    // Serializes the payload and writes it as a pending outbox row for OutboxRelay to publish.
