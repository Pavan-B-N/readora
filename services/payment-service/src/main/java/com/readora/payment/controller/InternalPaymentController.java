...
// Service-to-service only — protected by the shared gateway secret, no caller-identity check (see GatewaySecretFilter / SecurityProperties.public-routes); ownership of the order is already enforced by commerce-service, the only caller, before it ever reaches here, unlike PaymentController's public /api/v1/payments/{orderId} which authenticates the caller but doesn't check they own the order being asked about.
@Tag(name = "Internal")
@RestController
...
public class InternalPaymentController {
    ...
    // Returns payment details for the given order, regardless of caller identity.
    @GetMapping("/{orderId}")
