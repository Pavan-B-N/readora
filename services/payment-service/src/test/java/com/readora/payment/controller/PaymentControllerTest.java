...
class PaymentControllerTest {
    ...
    void getByOrder_found_returns200() throws Exception {
        when(paymentService.getByOrderId(any(), any())).thenReturn(new PaymentResponse(
        ...
        mockMvc.perform(get("/api/v1/payments/" + UUID.randomUUID())).andExpect(status().isOk());
    }

    @Test
    void getByOrder_notFound_mapsTo404() throws Exception {
        when(paymentService.getByOrderId(any(), any())).thenThrow(new PaymentNotFoundException());

        mockMvc.perform(get("/api/v1/payments/" + UUID.randomUUID())).andExpect(status().isNotFound());
    }
}
