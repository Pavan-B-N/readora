...
// Validates JWTs issued by auth-service using the shared signing secret — every other service only ever validates (auth-service issues, mcp-server mints its own short-lived internal tokens via issueInternalToken).
@Component
public class JwtService {

    private final SecretKey key;

    // Derives the HMAC signing key from the shared base64 secret.
    public JwtService(@Value("${app.jwt.secret}") String secret) {
        this.key = Keys.hmacShaKeyFor(Decoders.BASE64.decode(secret));
    }

    /**
     * Verifies a token's signature and expiry, and extracts the caller's user id from its subject claim.
     *
    ...
    /**
     * Extracts the caller's role codes from a token's claims. Only call this after extractUserId has confirmed the token verifies.
     *
    ...
    /**
     * Extracts the caller's email from a token's claims. Only call this after extractUserId has confirmed the token verifies.
     *
     * @param token a signed JWT string
    ...
    /**
     * Signs a short-lived (1 minute) token whose subject is the given user id — lets mcp-server pass an already-authenticated userId through a downstream service's normal JwtAuthenticationFilter without a separate internal-auth mechanism.
     *
    ...
    // Parses and signature-verifies a token, throwing JwtException if it's invalid, expired, or tampered.
