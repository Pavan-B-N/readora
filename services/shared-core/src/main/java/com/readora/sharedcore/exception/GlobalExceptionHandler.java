...
// Converts every uncaught exception into the shared ErrorResponse JSON shape, applied to all services via shared-core.
@RestControllerAdvice
public class GlobalExceptionHandler {

    private static final Logger log = LoggerFactory.getLogger(GlobalExceptionHandler.class);

    // Handles a deliberately thrown, already-classified business error.
    ...
    // Handles @Valid request-body binding failures, listing every invalid field.
    ...
    // Catch-all safety net for anything not already classified above.
    ...
    // Reuses the request's correlation ID if one was assigned, otherwise mints a fresh one.
