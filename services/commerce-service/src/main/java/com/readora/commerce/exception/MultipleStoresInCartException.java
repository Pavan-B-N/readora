...
// Physical browsing is store-scoped per customer, so this shouldn't happen in practice — a defensive guard against a stale cart spanning two stores (e.g. the customer switched stores mid-session without the cart being cleared).
