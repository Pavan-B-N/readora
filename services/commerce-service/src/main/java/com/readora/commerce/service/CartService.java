...
// Manages the caller's Redis-backed cart: adding/updating items with live catalog re-validation, and computing totals.
@Service
public class CartService {
    ...
    // Wires the cart repository, catalog client, and order-item repository.
    ...
    // Gets the caller's current cart with computed totals.
    public CartResponse getCart(UUID userId) {
        return toResponse(cartRepository.getItems(userId));
    }

    // Adds or increments a cart item after re-validating price/availability against catalog-service.
    public CartSummaryResponse addItem(UUID userId, AddCartItemRequest request) {
        ...
        // A virtual edition is a digital copy licensed once per purchase — there's no concept of "2 copies" of a file, so it's always exactly 1 regardless of what's requested or already in the cart, unlike physical stock which genuinely accumulates.
        int newQty;
        if (request.deliveryType() == DeliveryType.VIRTUAL) {
            // Nothing left to sell — re-adding (e.g. a "Buy again" click on order history, which doesn't know the difference between a physical rebuy and a virtual one) would let a customer pay twice for the exact same permanent, unlimited-access digital copy.
    ...
    // Sets a cart item's quantity directly (0 removes it), re-validating availability against catalog-service.
    public CartSummaryResponse setItemQty(UUID userId, UUID bookId, DeliveryType deliveryType, int qty) {
        ...
        if (qty == 0) {
            ...
            // No storeId here — this item was already store-validated when it was added, and a qty-only change can't move it to a different store, so re-checking against a store isn't meaningful (or available: the caller doesn't resend it for a qty bump).
    ...
    // Maps raw cart items to the full cart response with per-line and subtotal amounts.
    ...
    // Maps raw cart items to the lightweight summary totals.
