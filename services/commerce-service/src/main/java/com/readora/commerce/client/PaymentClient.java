...
// Direct service-to-service call to payment-service, bypassing api-gateway, deliberately with NO circuit breaker (best-effort only): refund status is a display enrichment for the admin returns view, not a security check, so a failure degrades to "unknown" rather than blocking the page (unlike UserServiceClient.getAdminStoreId, which fails closed).
@Component
public class PaymentClient {
    ...
    // Builds the internal RestClient with the gateway secret header.
    ...
    /**
     * Best-effort lookup of refund statuses for the given orders; failures are swallowed and logged.
     *
    ...
    // Best-effort, same rationale as getRefundStatuses above: transaction info is a display enrichment for the order-detail page, not something that should block the page if payment-service is briefly unavailable; empty also covers the normal case of an order whose payment record hasn't been created yet (e.g. right after checkout, before the order.created event has been consumed).
