...
// The one piece every order-lifecycle service (checkout, fulfillment, returns) shares: recording a status transition and publishing events through the transactional outbox — split out so those services depend on "how do I record a transition" rather than each duplicating history/outbox plumbing.
@Component
public class OrderEventRecorder {
    ...
    // Wires the history repository, outbox repository, and JSON mapper.
    ...
    // Serializes a payload and writes it to the outbox for later Kafka publication.
