...
// Direct service-to-service calls to catalog-service, bypassing api-gateway — sends the same shared secret the gateway stamps on requests so catalog-service's GatewaySecretFilter accepts it as a trusted internal caller; every method is wrapped with a "catalog-service" circuit breaker and retry (configured in application.yml), and since @TimeLimiter only applies to CompletableFuture-returning methods, the same configured duration instead bounds the underlying HTTP client's connect/read timeout directly, keeping it a single config value either way.
@Component
public class CatalogClient {
    ...
    // storeId is the caller's currently-delivering-from store, forwarded so catalog-service can report NOT_AVAILABLE_AT_STORE for a book stocked elsewhere rather than its raw inventory count; null is valid (e.g. a qty-only cart update already store-validated at add time).
    ...
    // Reserves stock for the requested items during checkout, translating not-found/conflict into domain exceptions.
    ...
    // Best-effort, unlike the checkout-critical methods above: cover images are a display enrichment for the order list, so a missing thumbnail degrades gracefully rather than blocking the page if catalog-service is briefly unavailable.
    ...
    // Circuit-breaker fallback for getBook; translates the failure via translate().
    private BookInfo getBookFallback(UUID bookId, UUID storeId, Throwable t) {
        throw translate(t);
    }

    // Circuit-breaker fallback for getStore; translates the failure via translate().
    private StoreInfo getStoreFallback(UUID storeId, Throwable t) {
        throw translate(t);
    }

    // Circuit-breaker fallback for reserveStock; translates the failure via translate().
    private ReserveStockResponse reserveStockFallback(ReserveStockRequest request, Throwable t) {
        throw translate(t);
    }

    // Circuit-breaker fallback for lookupVirtualEditions; translates the failure via translate().
    private VirtualEditionLookupResponse lookupVirtualEditionsFallback(VirtualEditionLookupRequest request, Throwable t) {
        throw translate(t);
    }

    // Converts a circuit-breaker-open rejection into the documented "unavailable" error; any other failure (a business error like BookNotFoundException, or a real transient failure once retries are exhausted) propagates unchanged.
