...
// The small chat that opens on a return while it's RETURN_REQUESTED — shared by both ReturnService (customer side) and AdminOrderService (admin side), each of which does its own ownership/store check before calling in here with an already-loaded, already-authorized Order.
@Service
public class ReturnMessageService {
    ...
    // Wires the message repository, user-service client, outbox repository, and JSON mapper.
    ...
    // Lists a return's chat thread, oldest first.
    ...
    // Posts a chat message; locked once a decision is made — nothing left to discuss once the return is approved/rejected.
    @Transactional
    ...
    // Notifies whoever didn't just send this message — the customer if an admin wrote it, or that store's admin if the customer did.
    ...
    // Maps a return-message entity to its response DTO.
