...
// Every way an order can end early and get refunded — cancellation before it ships, and the full return lifecycle (request, admin review, pickup, refund) after it's delivered; distinct from CheckoutService (creates the order), OrderQueryService (reads it), and OrderFulfillmentService (the forward path this reverses).
@Service
public class ReturnService {
    ...
    // Wires the order repository, user-service client, return-message service, and event recorder.
    ...
    // Cancels an order within its 48h/pre-shipment window and kicks off a refund.
    @Transactional
    ...
    // A virtual-only order has nothing physical to inspect, so it auto-advances straight through to REFUND_INITIATED in this same call — no admin review, no chat; an order with a physical item stops at RETURN_REQUESTED and waits for an admin decision (see reviewReturn()), with the refund not firing until the book is actually collected (updateReturnPickupStatus() -> RETURN_COLLECTED).
    @Transactional
    ...
    // The admin decision gate for a return with a physical item (a virtual-only return never reaches RETURN_REQUESTED for long enough to be reviewed — see returnOrder()); approving queues a pickup for delivery-agent-service (it listens for RETURN_APPROVED on order.status_changed), rejecting is terminal, no refund.
    @Transactional
    ...
    // Called by delivery-agent-service (via InternalDeliveryController) as an agent progresses a return pickup; enforces RETURN_APPROVED -> RETURN_ASSIGNED -> RETURN_EN_ROUTE -> RETURN_COLLECTED in order, and reaching RETURN_COLLECTED immediately kicks off the refund — mirrors OrderFulfillmentService.updateDeliveryStatus()'s shape exactly, just for the reverse (pickup) leg.
    @Transactional
    ...
    // Lists a return's chat thread, scoped to its owning user.
    ...
    // Posts a return-chat message as the customer, scoped to their own order.
    @Transactional
