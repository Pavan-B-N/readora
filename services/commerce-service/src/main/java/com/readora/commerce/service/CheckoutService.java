...
// Prices and creates a new order — the one entry point onto the order lifecycle every other order-lifecycle service (OrderQueryService, OrderFulfillmentService, ReturnService) picks up from.
@Service
public class CheckoutService {
    ...
    // Wires the order-related repositories, catalog/user-service clients, cart repository, and event recorder.
    ...
    // Prices and creates an order from the checkout request, reserving physical stock and looking up virtual editions, idempotent by idempotencyKey.
    @Transactional
    public CheckoutResponse checkout(UUID userId, String idempotencyKey, CheckoutRequest request) {
        ...
        // Belt-and-suspenders on top of CartService's own check at add-to-cart time — this is the actual point of no return (payment), so it's checked again here in case a caller ever reaches checkout without having gone through the cart (e.g. a direct API call).
    ...
    // Physical browsing is store-scoped per customer, so every physical line should share one store — this is a defensive check, not an expected path, given there's no legitimate way today to add items from two different stores to the same cart.
    ...
    // No stock to reserve — a digital copy doesn't deplete; every requested book must have an active virtual edition, priced at the virtual edition's own price (which can differ from the physical list price).
    ...
    // Publishes order.created via the outbox for downstream consumers.
    ...
    // Maps an order to its checkout response DTO.
    ...
    // Parses the requested payment method string, rejecting anything unrecognized.
    ...
    // Generates a human-readable order number: RDA-<year>-<random 6 digits>.
