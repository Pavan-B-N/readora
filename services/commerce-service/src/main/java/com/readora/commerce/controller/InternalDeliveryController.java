...
// Internal, service-to-service endpoints called by delivery-agent-service to fulfill orders and return pickups.
@Tag(name = "Internal")
@RestController
...
public class InternalDeliveryController {
    ...
    // Wires the query, fulfillment, and return services.
    ...
    // Gets an order's full delivery detail, including shipping address, for a delivery agent.
    @Operation(
    ...
    // Advances an order's delivery status through the CONFIRMED->ASSIGNED->SHIPPED->DELIVERED sequence.
    @Operation(
    ...
    // Advances an order's return-pickup status; reaching RETURN_COLLECTED also kicks off the refund.
    @Operation(
