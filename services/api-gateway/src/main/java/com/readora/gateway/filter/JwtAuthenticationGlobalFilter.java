...
public class JwtAuthenticationGlobalFilter implements GlobalFilter, Ordered {
    ...
    // Wires in the JWT validator and the configured public-routes list.
    ...
    // Whether the path is exempt from JWT validation.
    ...
    // Short-circuits the chain with a 401 response.
    private Mono<Void> reject(ServerWebExchange exchange) {
        exchange.getResponse().setStatusCode(HttpStatus.UNAUTHORIZED);
        return exchange.getResponse().setComplete();
    }

    // Runs after the gateway secret filter but before routing.
    @Override
    public int getOrder() {
        return -1;
    }
