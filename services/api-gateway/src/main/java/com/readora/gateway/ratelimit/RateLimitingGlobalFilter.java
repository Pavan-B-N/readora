...
// Enforces Redis-backed rate limits using route-specific rules and user ID or client IP keys; returns 429 Too Many Requests when a request exceeds its configured limit.
@Component
public class RateLimitingGlobalFilter implements GlobalFilter, Ordered {
    ...
    // Wires in the limiter, configured rules, JSON mapper, and JWT service used to key requests.
    ...
    // Checks the request against its route's rate limit rule and rejects it if exceeded.
    @Override
    ...
    // The matched gateway route's id, or a default if no route matched.
