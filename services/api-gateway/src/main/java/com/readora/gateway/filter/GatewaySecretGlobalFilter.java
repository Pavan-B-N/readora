...
public class GatewaySecretGlobalFilter implements GlobalFilter, Ordered {
    ...
    // Captures the configured gateway secret to stamp onto forwarded requests.
    public GatewaySecretGlobalFilter(@Value("${app.gateway.secret}") String gatewaySecret) {
        this.gatewaySecret = gatewaySecret;
    }

    // Stamps the gateway secret header onto the request before it continues down the chain.
    @Override
    ...
    // Runs before the JWT filter so downstream services always see the secret.
    @Override
    public int getOrder() {
        return -3;
    }
