...
// Wires a dedicated Lettuce connection for Bucket4j's distributed bucket state, kept separate from Spring Data Redis's own client since Bucket4j manages this connection directly with its own byte codec.
@Configuration
public class Bucket4jRedisConfig {

    // Creates the dedicated Redis client Bucket4j connects through.
    @Bean
    ...
    // Exposes the async proxy manager Bucket4j uses to build and consume distributed buckets.
    @Bean
