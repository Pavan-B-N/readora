...
// Service-to-service only — backs the /internal endpoints ai-service and commerce-service call directly (gateway-secret protected, not part of the public API surface).
@Service
public class InternalCatalogService {
    ...
    // Wires in the repositories this service depends on.
    ...
    // Looked up by commerce-service during checkout to validate a shipping address's city against the delivering store — commerce-service holds no local store data of its own.
    ...
    // Full text export for ai-service's embedding backfill; when needsReembeddingOnly is set, only books changed (or never embedded) since their last embedding are returned, so a repeat run doesn't re-embed the whole catalog.
    @Transactional(readOnly = true)
    ...
    // Filters a candidate list down to what's actually purchasable right now — a book with a virtual edition is available regardless of store, a physical-only book only when its fixed home store matches storeId and has stock — used by ai-service so the assistant never suggests an unbuyable title.
    @Transactional(readOnly = true)
    ...
    // Whether one book is purchasable — virtual edition regardless of store, or physical with stock at storeId.
