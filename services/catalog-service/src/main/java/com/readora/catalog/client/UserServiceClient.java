...
// Direct service-to-service call to user-service, bypassing api-gateway (same pattern as CommerceClient) — but unlike CommerceClient this backs a security check, so getAdminStoreId fails closed (surfaces a 503) while the other, best-effort methods below degrade to null/empty instead.
@Component
public class UserServiceClient {
    ...
    // Builds the RestClient with the gateway secret header and a configured connect/read timeout attached to every request.
    ...
    // Best-effort, unlike getAdminStoreId — a display name is cosmetic, so a failure degrades to null (reviews fall back to "Anonymous") rather than blocking the write.
