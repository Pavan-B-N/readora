...
// Called directly by commerce-service during checkout (internal, gateway-secret protected, not part of the public /api/v1 surface) to atomically check and reserve stock before an order is persisted — mirrors the doc's Flow 2 "reserve stock, price the order" step.
@Service
public class InventoryService {
    ...
    // Wires in the book and inventory repositories.
    public InventoryService(BookRepository bookRepository, InventoryRepository inventoryRepository) {
        this.bookRepository = bookRepository;
        this.inventoryRepository = inventoryRepository;
    }

    // Atomically checks and reserves stock for every requested item, throwing on the first unavailable one.
    @Transactional
