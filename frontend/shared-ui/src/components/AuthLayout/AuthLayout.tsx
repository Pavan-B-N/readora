    ...
    <div className={styles.page}>
      <div className={styles.leftPanel}>
        ...
        <div className={styles.messageArea}>
          {/* All frames stay mounted and just crossfade via the `animate` prop, rather than the more obvious AnimatePresence mount/unmount-per-key approach — that relies on exit-completion tracking to remove the outgoing frame, which React 18 StrictMode's double-invoked effects can leave desynced in dev (the frame never formally "exits" and lingers at its pre-animation state); nothing here is ever added or removed, so there's no exit lifecycle to get stuck on. */}
          {messages.map((m, i) => (
            <motion.div
              key={m.title}
