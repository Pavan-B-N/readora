...
/** Thunk that logs in and, on success, leaves the tokens/claims to be applied in the fulfilled reducer below. */
...
/** tokensReceived stores a fresh access/refresh token pair (from login or silent refresh); loggedOut clears the session. */
export const { tokensReceived, loggedOut } = authSlice.actions;
/** The auth slice's reducer — combines session state (tokens, email, roles) with the login thunk's loading status. */
