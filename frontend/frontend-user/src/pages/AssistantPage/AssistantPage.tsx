...
/** Full-page book assistant — sidebar of past conversations plus a chat pane, syncing the open conversation to the :conversationId route param. */
export function AssistantPage() {
  ...
  // The URL is the source of truth for which conversation is open — switching it (sidebar click, browser back/forward) re-syncs the session to match. loadConversation itself no-ops when the id is already the active one (see its comment) — the guard has to live there rather than here, since it needs conversationIdRef, not the conversationId state this effect would otherwise compare against (which can be a render or two behind).
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    // Auto-follow only while already near the bottom — someone scrolled up to reread history shouldn't get yanked back down by an incoming token.
  ...
  // Once a brand-new chat gets its first real conversationId back from the server, reflect it in the URL and refresh the sidebar so it shows up there too.
  useEffect(() => {
