...
// RETURN_REQUESTED is included here (not just the pre-return statuses) so the ebook "Read now" button stays visible while a return is pending admin review, matching the backend — which also doesn't revoke access until RETURN_APPROVED (see OrderItemRepository.findDistinctBookIdsByUserId).
...
/** Single order — status/return tracker, items, shipping/payment summary, return chat, and cancel/return actions; polls while a UPI payment is still settling. */
export function OrderDetailPage() {
  ...
  // A delivery-status push (e.g. "Out for delivery") should update this page immediately if it's already open, not just tick up the bell's unread count.
  ...
  const onSubmitReturn = async () => {
    ...
    // Physical items need a human to sign off before anything happens; a virtual-only order has nothing to inspect, so it auto-approves and starts refunding right away — see OrderService.returnOrder()'s javadoc.
  ...
  // An order's items can mix PHYSICAL and VIRTUAL editions — there's no single delivery type for the order as a whole, so every check below looks at the actual item lines instead of relying on order.deliveryType (which only ever reflects "has at least one physical item").
