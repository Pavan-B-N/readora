...
/** Checkout flow — shipping address selection/creation, wallet vs. UPI payment (UPI polls the order status until it resolves or times out), and order placement. */
  ...
  const resolveShippingAddress = async (): Promise<Address | null> => {
    ...
    if (newAddress) {
      ...
      try {
        ...
        // The endpoint only confirms { id, isDefault } — everything else is already known client-side from what was just submitted, so the full Address is built here rather than assumed to come back from the server.
