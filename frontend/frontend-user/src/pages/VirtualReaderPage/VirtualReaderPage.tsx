...
/** In-app PDF reader for an owned virtual edition — renders pages to a canvas (right-click disabled) and offers a book-grounded reading assistant chat alongside it. */
export function VirtualReaderPage() {
  ...
  // Tracks the viewer column's actual width so the page render below can fill it, rather than rendering at a fixed scale that leaves the extra room this page now has (full viewport width, no site-wide reading-column cap) unused.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    pdfRef.current.getPage(page).then(async (pdfPage) => {
      ...
      // Leaves a little breathing room either side rather than touching the card's own padding exactly, and never upscales a narrow page into a blurry giant on a very wide monitor.
