...
/** Book detail/PDP — cover, pricing and availability, add-to-cart/edition-picker flow, description, related reads, and reviews; swaps to a "Read now" prompt once the caller owns the virtual edition. */
export function BookDetailPage() {
  ...
  // Whether the signed-in caller already owns this book's virtual edition — if so, there's nothing left to sell them here; the purchase UI gives way to a "Read now" prompt instead.
  useEffect(() => {
    if (!accessToken) {
  ...
  // Keeps the browser tab and any share sheet that reads the live DOM (rather than a crawler hitting the server-rendered OG tags in server.mjs) in sync with the book being viewed.
  useEffect(() => {
    if (!book) return;
  ...
  // Once owned, the virtual edition is no longer something to buy — every purchase-flow check below (the picker modal, the add-to-cart click) should behave exactly as if it didn't exist.
  ...
  const onAddToCartClick = () => {
    // Only actually a "choice" when both editions are real options here — a virtual-only book (or one whose physical copy is out of stock / not stocked at this store) has exactly one buyable edition, so there's nothing to pick between and the modal should never appear.
        ...
        <div>
          ...
          <div className={styles.purchaseBox}>
            ...
            <div className={styles.purchaseRow}>
              {/* A virtual edition is a single digital copy — once it's in the cart there's nothing left to add, so the button would just look unresponsive on a second click. Only ever hidden for a virtual-only book; a mixed book still needs it for adding physical copies even after the virtual one's in the cart. */}
