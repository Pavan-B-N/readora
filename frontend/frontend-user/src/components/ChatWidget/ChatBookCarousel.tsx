...
export function ChatBookCarousel({ books }: ChatBookCarouselProps) {
  ...
  // The assistant's guardrail already restricts recommendations to books that are purchasable exactly one way at this store: in-stock physical, or a virtual edition when there's no store stock — never both at once in this catalogue, so there's nothing to ask the user to choose between; add whichever edition is actually the available one.
  ...
  return (
    <div className={styles.list}>
      {books.map((book) => {
        // A book recommended in chat could in principle have lines of both delivery types in cart already (e.g. added elsewhere) — arbitrarily surfaces whichever comes first rather than trying to represent both in this small a row.
        ...
        return (
          <div
            ...
            onKeyDown={(e) => {
              // Guard against a keydown bubbling up from a nested button (Add to cart, qty steppers) — only navigate when the row itself is the focused element.
