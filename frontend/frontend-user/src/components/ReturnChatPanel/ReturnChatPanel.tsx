...
/** The small back-and-forth with an admin while a return sits at RETURN_REQUESTED. */
  ...
  return (
    <div className={styles.panel}>
      <div className={styles.messages} ref={listRef}>
        {messages.length === 0 ? (
          <p className={styles.empty}>No messages yet — describe the issue below and an admin will respond here.</p>
        ) : (
          messages.map((message) => (
            ...
            >
              <span className={styles.messageSender}>{message.senderRole === 'ADMIN' ? 'Support' : 'You'}</span>
