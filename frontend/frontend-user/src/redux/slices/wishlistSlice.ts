...
/** Loads the caller's wishlist and stores it as a bookId → true lookup map. */
...
/** Adds a book to the wishlist on the server and locally. */
export const addWishlistItem = createAsyncThunk('wishlist/add', async (bookId: string) => {
  await addToWishlist(bookId);
  return bookId;
});

/** Removes a book from the wishlist on the server and locally. */
...
const wishlistSlice = createSlice({
  ...
  initialState,
  reducers: {
    /** Resets to an empty wishlist locally (e.g. on logout) without a server round-trip. */
