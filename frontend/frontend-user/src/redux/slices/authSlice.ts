...
/** Signs in with email/password and stores the returned tokens on success. */
...
/** Creates the account, then immediately logs in with the same credentials to obtain tokens. */
...
const authSlice = createSlice({
  ...
  initialState,
  reducers: {
    /** Stores a freshly-issued token pair (e.g. from a 401 refresh) and re-derives userId/email from the new access token's claims. */
    ...
    /** Clears the session, including the persisted refresh token. */
    loggedOut(state) {
