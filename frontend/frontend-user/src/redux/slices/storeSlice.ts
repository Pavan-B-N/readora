...
// Resolves "the store we're delivering from" — the signed-in caller's preferred store, or the default store (see pickDefaultStore) for anonymous callers or ones with no preference set yet. Every physical-book search requires this id, so callers (StoreSwitcher, HomePage) must wait for `resolved` before searching the physical tab.
...
/** Switches the active delivery store, persisting the preference server-side for signed-in callers. */
