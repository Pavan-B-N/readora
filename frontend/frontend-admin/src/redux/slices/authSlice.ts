...
/** Async thunk that calls the login API and, on success, populates state from the returned tokens. */
...
/** tokensReceived stores a fresh token pair (e.g. after a refresh) and decodes its claims; loggedOut clears all auth state and stored tokens. */
export const { tokensReceived, loggedOut } = authSlice.actions;
/** Reducer for the auth slice of the Redux store. */
